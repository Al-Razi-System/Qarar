...
import { Bell, BookOpenCheck, Building2, Calendar, ChevronDown, CircleHelp, ClipboardList, FileText, KeyRound, LayoutDashboard, LogOut, Menu, Search, Settings, ShieldCheck, Users, Workflow, X } from "lucide-react";
...
const navGroups = [
  ...
  { label: "الإدارة والتشغيل", items: [
    { href: "/admin/users", label: "المستخدمون", icon: Users, requiredAny: ["iam.users.read"] },
    { href: "/admin/permissions", label: "الأدوار والصلاحيات", icon: ShieldCheck, requiredAny: ["iam.roles.read", "iam.permissions.read"] },
    { href: "/admin/delegations", label: "التفويضات والإنابة", icon: KeyRound, requiredAny: ["iam.roles.assign"] },
    { href: "/admin/sessions", label: "مراقبة الجلسات", icon: Workflow, requiredAny: ["iam.sessions.manage"] },
    { href: "/admin/audit", label: "سجل التدقيق", icon: ClipboardList, requiredAny: ["audit.logs.read"] },
  ]},
  { label: "إعدادات الاجتماعات", items: [
    { href: "/admin/settings/meeting-types", label: "أنواع الاجتماعات", icon: Calendar, adminOnly: true },
  ]},
];
...
export type AppAccessContext = {
  ...
  permissions?: string[];
  roles?: Array<{ code?: string; name_ar?: string; membership_title?: string | null }>;
};

type NavigationProps = { pathname: string; access: AppAccessContext; onNavigate?: () => void };

function hasAdministrativeRole(access: AppAccessContext) {
  return Boolean(access.is_system_admin || access.roles?.some((role) => role.code === "governance_admin"));
}

function canOpenItem(access: AppAccessContext, item: { href: string; requiredAny?: string[]; adminOnly?: boolean }) {
  if (access.is_system_admin) return true;
  if (item.adminOnly) return hasAdministrativeRole(access);
  return !item.requiredAny?.length || item.requiredAny.some((permission) => access.permissions?.includes(permission));
}

function Navigation({ pathname, access, onNavigate }: NavigationProps) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col overflow-y-auto px-3 py-5">
    {navGroups.map((group) => ({ ...group, items: group.items.filter((item) => canOpenItem(access, item)) })).filter((group) => group.items.length).map((group) => <div key={group.label} className="mb-6">
...
function accountInitials(name: string) {
  return name.trim().split(/\s+/).slice(0, 2).map((part) => part[0] ?? "").join("") || "ق";
}

export function AppShell({ children, access }: { children: ReactNode; access: AppAccessContext }) {
  ...
  const displayName = access.full_name_ar?.trim() || access.full_name_en?.trim() || access.email || "المستخدم الحالي";
  const initials = accountInitials(displayName);
  const primaryRole = [...(access.roles ?? [])].sort((left, right) => {
    const priority: Record<string, number> = { governance_admin: 0, council_chair: 1, council_rapporteur: 2, council_member: 3 };
    return (priority[left.code ?? ""] ?? 99) - (priority[right.code ?? ""] ?? 99);
  })[0];
  const roleLabel = access.is_system_admin
    ? "مسؤول النظام"
    : primaryRole?.membership_title
      || primaryRole?.name_ar
      || access.job_title
      || "مستخدم النظام";
  ...
  const canAccessCurrentPath = !currentItem || canOpenItem(access, currentItem);

  useEffect(() => {
    if (!canAccessCurrentPath) router.replace("/admin");
  ...
  if (!canAccessCurrentPath) {
    return <div dir="rtl" className="grid min-h-screen place-items-center bg-[#f4f7fb] text-sm font-bold text-[#52647a]">جارٍ توجيهك إلى مساحة العمل المتاحة لحسابك…</div>;
  }
  ...
  return <div dir="rtl" className="min-h-screen bg-[#f4f7fb] text-[#14233a]">
    ...
    <aside className={`fixed inset-y-0 right-0 z-50 flex w-[min(86vw,320px)] flex-col border-l border-[#dbe5ef] bg-white text-[#17283f] shadow-[-8px_0_30px_rgba(15,42,72,.12)] transition-transform duration-200 lg:hidden ${mobileNavigationOpen ? "translate-x-0" : "translate-x-full"}`}>
      ...
      <Navigation pathname={pathname} access={access} onNavigate={() => setMobileNavigationOpen(false)} />
    ...
    <aside className="fixed inset-y-0 right-0 z-30 hidden w-[272px] flex-col border-l border-[#dbe5ef] bg-white text-[#17283f] shadow-[-8px_0_30px_rgba(15,42,72,.04)] lg:flex">
      ...
      <Navigation pathname={pathname} access={access} />
      <div className="mt-auto border-t border-[#e8eef4] bg-[#fbfcfe] p-4"><div className="mb-3 flex items-center gap-2 rounded-xl px-2 text-[11px] text-[#6f8095]"><Settings size={15} /> إعدادات النظام <CircleHelp className="mr-auto" size={15} /></div><div className="flex items-center gap-2.5 rounded-xl border border-[#dce6ef] bg-white p-2.5 shadow-sm"><div className="grid h-9 w-9 place-items-center rounded-xl bg-[#0872df] text-xs font-black text-white">{initials}</div><div className="min-w-0 flex-1"><p className="truncate text-[11px] font-bold text-[#22344c]">{displayName}</p><p className="mt-0.5 truncate text-[10px] text-[#8998aa]">{roleLabel}</p></div><button type="button" onClick={() => void logout()} disabled={loggingOut} aria-label="تسجيل الخروج" className="grid h-8 w-8 place-items-center rounded-lg text-[#8191a4] hover:bg-[#f1f6fb] hover:text-[#0066cc] disabled:opacity-60"><LogOut size={15} /></button></div></div>
    </aside>
    <div className="lg:mr-[272px]">
      <header className="sticky top-0 z-20 flex h-[72px] items-center gap-3 border-b border-[#e1e9f2] bg-white/95 px-4 backdrop-blur-xl sm:px-5 lg:h-[82px] lg:px-8"><button type="button" onClick={() => setMobileNavigationOpen(true)} className="grid h-10 w-10 place-items-center rounded-xl text-[#52647a] hover:bg-[#f2f6fa] lg:hidden" aria-label="فتح القائمة"><Menu size={23} /></button><form onSubmit={submitSearch} className="relative hidden w-full max-w-[420px] sm:block"><Search size={17} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-[#8b9aad]" /><input value={search} onChange={(event) => setSearch(event.target.value)} aria-label="بحث في الموضوعات" placeholder="ابحث في الموضوعات…" className="h-11 w-full rounded-xl border border-[#e3eaf2] bg-[#f8fafc] pr-10 pl-4 text-xs outline-none focus:border-[#9bc9f2] focus:bg-white focus:ring-4 focus:ring-[#0066cc]/5" /></form><div className="mr-auto flex items-center gap-2">
        ...
        <div className="relative"><button type="button" onClick={() => { setProfileOpen((open) => !open); setNotificationsOpen(false); }} aria-expanded={profileOpen} className="hidden items-center gap-2 rounded-xl border border-[#e5ebf2] px-3 py-2 sm:flex"><span className="grid h-7 w-7 place-items-center rounded-lg bg-[#e7f2ff] text-[10px] font-black text-[#0066cc]">{initials}</span><span className="max-w-40 truncate text-xs font-bold text-[#23334b]">{displayName}</span><ChevronDown size={14} className="text-[#8b9aad]" /></button>{profileOpen && <div className="absolute left-0 top-12 z-30 w-60 rounded-2xl border border-[#dce7f1] bg-white p-2 shadow-xl"><div className="border-b border-[#edf2f6] px-3 py-2"><p className="truncate text-[11px] font-bold text-[#24364d]">{displayName}</p><p className="mt-1 truncate text-[10px] text-[#718196]">{access.email}</p><p className="mt-1 text-[10px] font-bold text-[#0872df]">{roleLabel}</p></div><button type="button" onClick={() => void logout()} disabled={loggingOut} className="mt-1 flex h-9 w-full items-center gap-2 rounded-xl px-3 text-right text-[11px] font-bold text-[#52647a] hover:bg-[#f2f6fa] hover:text-[#0066cc]"><LogOut size={15} />{loggingOut ? "جارٍ الخروج…" : "تسجيل الخروج"}</button></div>}</div>
      </div></header>
