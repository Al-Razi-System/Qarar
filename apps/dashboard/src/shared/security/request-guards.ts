...
function configuredDashboardOriginAliases(
  environment: Environment = process.env,
): Set<string> {
  const aliases = new Set<string>();

  for (const rawAlias of (environment.APP_ORIGIN_ALIASES ?? "").split(",")) {
    const alias = configuredDashboardOrigin({
      ...environment,
      APP_ORIGIN: rawAlias.trim(),
    });
...
function isEquivalentDevelopmentLoopback(
  origin: string,
...
export function rejectUntrustedMutation(
  ...
  const allowedAliases = configuredDashboardOriginAliases(environment);
  if (
    !origin ||
    origin === "null" ||
    (origin !== allowedOrigin &&
      !allowedAliases.has(origin) &&
      !isEquivalentDevelopmentLoopback(origin, allowedOrigin, environment))
  ) {
