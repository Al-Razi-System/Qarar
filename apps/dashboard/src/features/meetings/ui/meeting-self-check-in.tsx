...
async function selfCheckIn(meetingId: string, token: string) {
  const response = await fetch("/api/admin/meetings", {
    ...
    body: JSON.stringify({
      ...
      params: { p_meeting_id: meetingId, p_token: token, p_device_label: "dashboard-self-check-in" },
    }),
  });
...
export function MeetingSelfCheckIn() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      await selfCheckIn(normalizedMeetingId, tokenForMeeting(token, normalizedMeetingId));
