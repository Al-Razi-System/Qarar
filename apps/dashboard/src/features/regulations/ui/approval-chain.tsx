  ...
  return <section id="approval-workflow" aria-labelledby="approval-chain-title" className="scroll-mt-28 overflow-hidden rounded-2xl border border-[#dbe6f1] bg-white shadow-[0_12px_34px_rgba(15,42,72,.06)]">
    ...
    <div className="grid min-h-[470px] xl:grid-cols-[270px_minmax(0,1fr)]">
      ...
      {selected && <div role="tabpanel" aria-live="polite" className="min-w-0 p-5 sm:p-7">
        ...
        {selected.locked ? <button type="button" disabled title="أكمل المرحلة السابقة قبل الانتقال" className="mt-6 flex h-11 w-full cursor-not-allowed items-center justify-center gap-2 rounded-xl bg-[#e2e8ef] text-[10px] font-black text-[#77889b]"><LockKeyhole size={14} />أكمل المرحلة السابقة أولًا</button> : selected.actionHref ? <Link href={selected.actionHref} title={selected.actionLabel || "فتح أدوات المرحلة"} onClick={() => onSelect(selectedIndex)} className="mt-6 flex h-11 w-full items-center justify-between rounded-xl bg-[#0872df] px-4 text-[10px] font-black text-white shadow-[0_8px_18px_rgba(0,102,204,.18)] transition hover:bg-[#0066cc]"><span>{selected.actionLabel || (selected.done ? "مراجعة المرحلة" : "بدء التنفيذ")}</span><ArrowLeft size={15} /></Link> : <button type="button" title={selected.actionLabel || "فتح أدوات المرحلة"} onClick={() => onSelect(selectedIndex)} className="mt-6 flex h-11 w-full items-center justify-between rounded-xl bg-[#0872df] px-4 text-[10px] font-black text-white"><span>{selected.actionLabel || "فتح المرحلة"}</span><ArrowLeft size={15} /></button>}
      </div>}
    </div>
  </section>;
}
