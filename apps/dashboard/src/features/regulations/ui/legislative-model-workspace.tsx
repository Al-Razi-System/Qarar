...
const tabLabels: Record<Tab, string> = {
  structure: "1. المادة المختارة",
  rules: "2. ماذا تنفذ المادة؟",
  references: "العلاقات القانونية",
  readiness: "هل الإصدار جاهز؟",
  compare: "ما الذي تغيّر؟",
};
const tabGuides: Record<
  Tab,
  { question: string; description: string; result: string }
> = {
  structure: {
    question: "ما النص النظامي الذي نعمل عليه؟",
    description:
      "استعرض الأبواب والفصول والمواد والبنود، ثم اختر مادة لقراءة نصها أو تعديل بياناتها.",
    result: "النتيجة: مادة محددة وواضحة المصدر داخل الإصدار.",
  },
  rules: {
    question: "كيف تتحول المادة إلى سلوك داخل النظام؟",
    description:
      "حدد الشرط والمسؤول والنتيجة والمسار الذي يجب أن ينفذه النظام عند انطباق المادة.",
    result: "النتيجة: قاعدة قابلة للفحص والتنفيذ مرتبطة بمصدرها القانوني.",
  },
  references: {
    question: "ما علاقة هذه المادة بنص أو قرار آخر؟",
    description:
      "سجل أن المادة تستند إلى نص آخر أو تعدله أو تلغيه أو تفسره. هذه ليست إحالة معاملة بين المجالس.",
    result: "النتيجة: سلسلة استناد قانونية قابلة للتدقيق.",
  },
  readiness: {
    question: "هل اكتملت متطلبات الإرسال للمراجعة؟",
    description:
      "يفحص النظام المحتوى والنطاق والقواعد والمصدر ويعرض الأخطاء والتحذيرات، لكنه لا يعتمد الإصدار.",
    result: "النتيجة: قائمة نواقص عملية أو تأكيد الجاهزية للمراجعة.",
  },
  compare: {
    question: "ما الفرق بين النسخة الحالية والسابقة؟",
    description:
      "يعرض المواد المضافة والمعدلة والملغاة بين إصدارين من اللائحة نفسها.",
    result: "النتيجة: أثر التعديل القانوني قبل اعتماده.",
  },
};
  ...
  useEffect(() => {
    void Promise.all([
      ...
      .then(([unitData, classData, workflowData]) => {
        ...
        setWorkflows(workflowTemplatesFromResponse(workflowData));
      })
      .catch(() => undefined);
  }, []);
  ...
  return (
    <>
      ...
      >
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-[10px] leading-5 text-blue-100">
                افصل النص الرسمي عن تفسيره الرقمي، وحدد الشروط والمتطلبات
                والجهات والنتائج والمسارات مع إمكانية التتبع والمقارنة.
              </p>
            </div>
        ...
        {!rulesOnly && (
          <nav
            aria-label="خطوات تحويل النص النظامي إلى قواعد"
            role="tablist"
            className="flex gap-1 overflow-x-auto border-b border-[#e6edf4] bg-[#f8fafc] p-2"
          >
            {(Object.keys(tabLabels) as Tab[]).map((value) => (
              <button
                ...
                title={tabGuides[value].description}
        ...
        {!rulesOnly && (
          <div className="border-b border-[#dce8f2] bg-[#f3f9ff] px-4 py-3">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <strong className="text-[10px] font-black text-[#17466f]">
                  {tabGuides[tab].question}
                </strong>
                <p className="mt-1 max-w-3xl text-[9px] leading-5 text-[#5c7389]">
                  {tabGuides[tab].description}
                </p>
              </div>
              <span className="rounded-xl border border-[#cfe3f4] bg-white px-3 py-2 text-[8px] font-black text-[#356587]">
                {tabGuides[tab].result}
              </span>
            </div>
          </div>
        )}
        ...
        {!model ? (
          ...
          <div className="p-4">
                                    ...
                                    rpc(
                                      "admin_update_policy_version_legal_metadata",
                                      {
                                        ...
                                        p_approval_authority:
                                          versionForm.approval_authority ||
                                          null,
                                        p_approval_decision_number:
                                          versionForm.approval_decision_number ||
                                          null,
                                        p_approval_date:
                                          versionForm.approval_date || null,
                                        p_issue_reason:
                                          versionForm.issue_reason || null,
                                        p_supersedes_version_id:
                                          versionForm.supersedes_version_id ||
                                          null,
                                        p_source_document_hash:
                                          versionForm.source_document_hash ||
                                          null,
                                      },
                                    ),
            ...
            {tab === "rules" && selectedItem && (
              <div className="space-y-4">
                <div className="grid gap-2 rounded-2xl border border-[#d7e7f4] bg-[#f7fbff] p-3 sm:grid-cols-4">
                  {[
                    "1. متى تعمل القاعدة؟",
                    "2. ما الشرط المطلوب؟",
                    "3. من المسؤول؟",
                    ...
                    <div
                      key={step}
                      className="rounded-xl bg-white px-3 py-2 text-center text-[8px] font-black text-[#315d7f] shadow-sm"
                    >
                      {step}
                    </div>
                  ))}
                  ...
                  <aside className="min-w-0 rounded-2xl border border-[#dbe6f0] bg-white p-4 shadow-[0_10px_28px_rgba(25,67,110,.05)]">
                    ...
                    {!selectedItemCanHaveRules && (
                      <div className="mb-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-[9px] leading-5 text-amber-800">
                        ...
                        والفصول تُستخدم للتنظيم فقط ولا تحمل قواعد تشغيلية
                        مباشرة.
                      </div>
                    )}
            ...
            {tab === "references" && selectedItem && (
              <div className="space-y-4">
                <div className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-[9px] leading-6 text-blue-900">
                  <strong>تنبيه مهم:</strong> الإحالة هنا علاقة بين نصوص
                  قانونية، مثل «مستند إلى» أو «يلغي». نقل موضوع من مجلس إلى مجلس
                  آخر يُضبط داخل القاعدة التنفيذية ومسار الاعتماد، وليس في هذا
                  التبويب.
                </div>
                  ...
                  <section className="rounded-xl border border-[#e0e8f0] p-4">
                    ...
                    <div className="mt-4 space-y-3">
                      <Field label="نوع العلاقة">
                        ...
                        >
                          {Object.entries(referenceLabels).map(
                            ([value, label]) => (
              ...
              <section className="rounded-xl border border-[#e0e8f0] p-4">
                <div className="mb-4 grid gap-2 sm:grid-cols-3">
                  <ReadinessStep
                    number="1"
                    title="يفحص"
                    text="المحتوى والنطاق والقواعد والمصدر"
                  />
                  <ReadinessStep
                    number="2"
                    title="يعرض"
                    text="الأخطاء المانعة والتحذيرات"
                  />
                  <ReadinessStep
                    number="3"
                    title="لا يعتمد"
                    text="الاعتماد قرار مستخدم مخوّل بعد الفحص"
                  />
                </div>
      ...
      {ruleDrawerOpen && selectedItem && selectedItemCanHaveRules && (
        <div
          className="fixed inset-0 z-50 bg-[#081630]/45 backdrop-blur-sm"
          role="dialog"
          aria-modal="true"
          aria-label="إدارة قواعد المادة"
        >
          <aside
          ...
          >
            <header className="flex items-start justify-between gap-4 border-b border-[#e5edf4] bg-[linear-gradient(135deg,#0a1330,#0066cc)] px-5 py-5 text-white">
              <div>
                <p className="text-[10px] font-black text-[#ffb46c]">
                  إدارة تشريعية
                </p>
                <h3 className="mt-1 text-base font-black">
                  قواعد المادة: {selectedItem.title_ar}
                </h3>
                <p className="mt-1 text-[10px] text-white/75">
                  يمكن للمادة الواحدة أن تحتوي عدة قواعد مستقلة، لكل منها شروط
                  ومتطلبات وصلاحيات ونتائج.
                </p>
              </div>
              <button
                onClick={() => setRuleDrawerOpen(false)}
                className="grid h-9 w-9 place-items-center rounded-xl border border-white/20 bg-white/10 text-lg"
                aria-label="إغلاق لوحة قواعد المادة"
              >
                ×
              </button>
            </header>
            <div className="flex-1 overflow-y-auto p-5">
              <div className="mb-4 flex items-center justify-between rounded-2xl border border-[#dce9f5] bg-[#f8fbff] p-4">
                <div>
                  <p className="text-[10px] font-black text-[#0066cc]">
                    {selectedItem.item_code}
                  </p>
                  <p className="mt-1 text-xs font-bold text-[#203952]">
                    {selectedItem.rules?.length ?? 0} قاعدة مرتبطة بهذه المادة
                  </p>
                </div>
                {editable && (
                  <button
                    onClick={() => {
                      setRuleForm(emptyRule(selectedItem));
                      setTab("rules");
                      setRuleDrawerOpen(false);
                    }}
                    className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#0066cc] px-4 text-[10px] font-black text-white"
                  >
                    <Plus size={15} /> إنشاء قاعدة
                  </button>
                )}
              </div>
              <div className="space-y-3">
                {(selectedItem.rules ?? []).map((rule) => (
                  <article
                    key={rule.id}
                    className="rounded-2xl border border-[#e2eaf2] p-4"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h4 className="text-xs font-black text-[#1e3854]">
                          {rule.name_ar}
                        </h4>
                        <p className="mt-1 text-[10px] text-[#718399]">
                          {ruleTypeLabels[rule.rule_type] ?? rule.rule_type} ·
                          أولوية {rule.priority}
                        </p>
                        {rule.description && (
                          <p className="mt-2 text-[11px] leading-6 text-[#526f8b]">
                            {rule.description}
                          </p>
                        )}
                      </div>
                      {editable && (
                        <button
                          ...
                          className="shrink-0 rounded-lg border border-[#c9def1] px-3 py-2 text-[10px] font-black text-[#0066cc]"
                        >
                          تحرير
                        </button>
                      )}
                ...
                {!selectedItem.rules?.length && (
                  <div className="rounded-2xl border border-dashed border-[#bdd5e8] bg-[#fbfdff] p-8 text-center">
                    <Gavel className="mx-auto text-[#83a5c5]" size={28} />
                    <h4 className="mt-3 text-xs font-black text-[#29435e]">
                      لا توجد قواعد لهذه المادة بعد
                    </h4>
                    <p className="mt-2 text-[11px] leading-6 text-[#718399]">
                      أنشئ القاعدة الأولى لتحديد الشروط والمتطلبات والجهات
                      المخولة والقرارات والمسار.
                    </p>
                  </div>
                )}
              </div>
            </div>
            <footer className="border-t border-[#e5edf4] bg-[#fbfdff] p-4">
              <button
                onClick={() => {
                  setTab("rules");
                  setRuleDrawerOpen(false);
                }}
                className="w-full rounded-xl border border-[#cbddeb] px-4 py-3 text-[11px] font-black text-[#365572]"
              >
                فتح مساحة القواعد الكاملة
              </button>
            </footer>
          </aside>
        </div>
...
function ReadinessStep({
  ...
  return (
    <div className="flex items-start gap-2 rounded-xl border border-[#dce8f2] bg-[#f8fbff] p-3">
      <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-[#0872df] text-[8px] font-black text-white">
        {number}
      </span>
      <div>
        <strong className="text-[9px] font-black text-[#294863]">
          {title}
        </strong>
        <p className="mt-1 text-[8px] leading-5 text-[#6d8092]">{text}</p>
      </div>
    </div>
  );
}
