...
export async function GET() {
  ...
  const user = await response.json() as { factors?: Array<{ id: string; factor_type: string; status: string; friendly_name?: string }> };
  return NextResponse.json({ factors: (user.factors ?? []).filter((factor) => factor.factor_type === "totp" && factor.status === "verified") }, { headers: { "Cache-Control": "no-store" } });
}
