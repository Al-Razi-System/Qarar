...
const statusLabels: Record<string, string> = {
  draft: "مسودة", new: "جديد", pending_review: "بانتظار المراجعة", under_review: "قيد المراجعة",
  approved: "معتمد", rejected: "مرفوض", needs_info: "يحتاج استكمال", referred: "محال",
...
function Summary({ topic, workflow, preview }: { topic: TopicDetail; workflow: Workflow | null; preview: RegulationPreview | null }) {
  return <div className="space-y-6"><SectionTitle icon={ClipboardCheck} title="ملخص الموضوع" description="نظرة سريعة على بيانات الموضوع وحالته الحاكمة الحالية." /><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><Info label="الجهة" value={topic.governance_unit?.name_ar ?? "—"} /><Info label="الفئة" value={topic.category?.name_ar ?? "—"} /><Info label="الأولوية" value={priorityLabels[topic.priority] ?? topic.priority} /><Info label="حالة المسار" value={statusLabels[workflow?.status ?? topic.routing_status ?? "not_started"] ?? workflow?.status ?? topic.routing_status ?? "لم يبدأ"} /></div><div className="rounded-2xl border border-[#e4edf5] bg-[#fbfdff] p-4"><h3 className="text-xs font-black text-[#0a1330]">الوصف</h3><p className="mt-2 whitespace-pre-wrap text-xs leading-7 text-[#52647a]">{topic.description || "لم يُسجل وصف للموضوع."}</p></div>{preview?.workflow?.description && <div className="rounded-2xl border border-blue-100 bg-[#f2f8ff] p-4 text-xs leading-6 text-[#31516f]"><strong className="text-[#0066cc]">أثر اللائحة على الموضوع: </strong>{preview.workflow.description}</div>}</div>;
}
