...
export function TopicRegulationCreator({ onFollowTopic }: TopicRegulationCreatorProps) {
  ...
  const canMoveToRegulation = hasTopicData && Boolean(form.unit) && Boolean(form.category) && !loadingReferences && !busy;
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-[#e2e9f1] bg-[#fbfdff] p-4">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <Field label="الجهة أو المجلس" hint={!loadingReferences && !references.units.length ? "لا توجد جهة تملك صلاحية إنشاء موضوع فيها ضمن صلاحياتك الحالية." : "تظهر الجهات التي تملك صلاحية إنشاء موضوع فيها فقط."}>
                <select aria-label="الجهة أو المجلس" aria-invalid={!loadingReferences && !references.units.length} disabled={loadingReferences || !references.units.length} className={invalidField(input, !loadingReferences && !references.units.length)} value={form.unit} onChange={(event) => resetOptions({ ...form, unit: event.target.value })}>
                  <option value="">{loadingReferences ? "جارٍ تحميل الجهات…" : references.units.length ? "اختر الجهة أو المجلس" : "لا توجد جهات متاحة"}</option>
                  {references.units.map((unit) => <option key={unit.id} value={unit.id}>{unit.name_ar}</option>)}
                </select>
              </Field>
              <Field label="فئة الموضوع" hint={!loadingReferences && !references.categories.length ? "لا توجد فئات موضوعات نشطة حاليًا." : "اختر الفئة التي تصف الطلب بدقة."}>
                <select aria-label="فئة الموضوع" aria-invalid={!loadingReferences && !references.categories.length} disabled={loadingReferences || !references.categories.length} className={invalidField(input, !loadingReferences && !references.categories.length)} value={form.category} onChange={(event) => resetOptions({ ...form, category: event.target.value })}>
                  <option value="">{loadingReferences ? "جارٍ تحميل الفئات…" : references.categories.length ? "اختر فئة الموضوع" : "لا توجد فئات متاحة"}</option>
                  {references.categories.map((category) => <option key={category.id} value={category.id}>{category.name_ar}</option>)}
                </select>
              </Field>
              ...
              <Field label="تاريخ المطابقة" hint="يُستخدم للتحقق من أن اللائحة كانت نافذة في هذا التاريخ."><input aria-label="تاريخ المطابقة" type="date" className={input} value={form.effectiveOn} onChange={(event) => resetOptions({ ...form, effectiveOn: event.target.value })}/></Field>
...
{reviewReady && <section className="mt-4 overflow-hidden rounded-2xl border border-[#0a1330]/10 bg-white shadow-[0_16px_36px_rgba(10,19,48,.12)]">
              ...
              <div className="grid gap-3 p-4 lg:grid-cols-2">
                ...
                <ReviewItem title="الجهة والفئة" value={selectedUnit?.name_ar ?? "—"} hint={selectedCategory?.name_ar ?? "—"}/>
                ...
                <ReviewItem title="أول جهة مسؤولة" value={firstRouteStep?.responsible_entity || "ستحدد عند بدء المسار"} hint={firstRouteStep?.responsible_role || "—"}/>
              </div>
