...
import {
  AlertCircle, Calendar, Check, Clock, LoaderCircle, MapPin, Play, Plus, Search, Users, X,
} from "lucide-react";
...
type MeetingDetail = Meeting & {
  ...
  agenda_items?: Array<{
    ...
    topic?: { id: string; title_ar: string; topic_no?: string; status?: string };
    is_exception?: boolean;
    exception_reason?: string | null;
  }>;
};

type MinuteApproval = { id: string; name_ar?: string; approval_status: string; notes?: string | null; updated_at: string; user_id?: string };
type MeetingMinutes = {
  id: string; content_draft?: string | null; content_final?: string | null; status: string;
  updated_at: string; approvals?: MinuteApproval[];
};
type MeetingReadiness = { ready: boolean; checks: Array<{ code: string; label: string; complete: boolean; count?: number }> };

type AgendaCandidate = { id: string; title_ar: string; topic_no?: string; priority?: string; current_step?: string | null; responsibility?: string | null };

type Notice = { kind: "success" | "error"; text: string };
type MeetingReference = { id: string; name_ar: string; code?: string };
type MeetingFormOptions = {
  meeting_units?: MeetingReference[];
  meeting_types?: MeetingReference[];
  location_types?: string[];
};

const statusLabels: Record<string, string> = {
  ready_to_start: "جاهزة للبدء", in_progress: "منعقدة",
  waiting_for_minutes: "بانتظار المحضر", waiting_for_approval: "بانتظار الاعتماد", archived: "مؤرشف",
  draft: "مسودة", scheduled: "مجدولة", invited: "تم الدعوة",
  in_session: "منعقدة", closed: "مغلقة", cancelled: "ملغاة",
};
const statusTone: Record<string, string> = {
  ready_to_start: "bg-purple-50 text-purple-700", in_progress: "bg-emerald-50 text-emerald-700",
  waiting_for_minutes: "bg-amber-50 text-amber-700", waiting_for_approval: "bg-orange-50 text-orange-700",
  draft: "bg-slate-100 text-slate-600",
  scheduled: "bg-blue-50 text-blue-700",
  invited: "bg-purple-50 text-purple-700",
  in_session: "bg-emerald-50 text-emerald-700",
  closed: "bg-[#eef1f5] text-[#6d7b8e]",
  cancelled: "bg-red-50 text-red-700",
};

function isAgendaEditable(status?: string) {
...
export function MeetingsWorkspace() {
  ...
  useEffect(() => { void loadMeetings(); }, [query]);

  async function loadMeetings() {
    setLoading(true); setNotice(null);
    try {
      const result = await rpc<{ items: Meeting[]; total: number }>("search_meetings", {
        p_query: query || null, p_status: null, p_unit_id: null,
  ...
  async function respondApproval(approval: MinuteApproval, decision: "approve" | "return") {
    if (!selected) return;
    const notes = decision === "return" ? window.prompt("سبب إعادة المحضر للمراجعة:") : null;
    if (decision === "return" && !notes) return;
    setMinutesLoading(true); setNotice(null);
    try {
      await rpc("respond_meeting_minutes_approval", { p_approval_id: approval.id, p_decision: decision, p_notes: notes, p_expected_updated_at: approval.updated_at });
      await openDetail(selected.id); await loadMeetings();
      setNotice({ kind: "success", text: decision === "approve" ? "تمت المصادقة على المحضر." : "أُعيد المحضر للمراجعة." });
  ...
  async function loadAgendaCandidates(queryOverride?: string) {
    ...
    try {
      const result = await rpc<{ items: AgendaCandidate[]; locked?: boolean }>("search_eligible_agenda_topics", {
  ...
  async function openLiveSession() {
    ...
    try {
      ...
      window.location.assign(`/admin/meetings/${selected.id}/live`);
    } catch (err) {
      ...
      <div className="flex items-center justify-between rounded-2xl border border-[#e2e9f1] bg-white p-4 shadow-[0_3px_16px_rgba(24,48,80,.035)]">
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && loadMeetings()} placeholder="ابحث في الاجتماعات..." className="h-10 w-64 rounded-xl border border-[#dfe7ef] bg-[#fafcfe] pr-9 pl-3 text-xs outline-none focus:border-[#9bc9f2]" />
          </div>
            ...
            <div className="divide-y divide-[#edf1f5]">
              <div className="p-5">
                <div className="mb-3 flex items-center justify-between gap-3">
                  ...
                  <button onClick={() => void openAgendaModal()} disabled={agendaLoading || !isAgendaEditable(selected.status)} title={isAgendaEditable(selected.status) ? undefined : "يُقفل جدول الأعمال بعد بدء الاجتماع"} className="rounded-xl border border-[#cfe0f0] px-3 py-2 text-[10px] font-bold text-[#0066cc] disabled:cursor-not-allowed disabled:border-[#e4e9ef] disabled:bg-[#f5f7fa] disabled:text-[#94a1b0] disabled:opacity-100">{agendaLoading ? "جارٍ التحميل…" : "إضافة موضوع"}</button>
                </div>
              ...
              {/* Agenda */}
              <div className="p-5">
                <h3 className="mb-3 text-xs font-black text-[#0a1330]">جدول الأعمال ({selected.agenda_items?.length ?? 0} بند)</h3>
                {selected.agenda_items?.length ? (
                  <div className="space-y-2">
                    {selected.agenda_items.map((item, index) => (
                      <div key={item.id} className="flex items-center gap-3 rounded-xl border border-[#edf2f7] bg-[#fbfdff] px-3 py-2.5">
                        <span className="grid h-7 w-7 place-items-center rounded-lg bg-[#e7f2ff] text-[10px] font-black text-[#0066cc]">{item.agenda_order}</span>
                        <span className="flex-1 text-xs font-bold text-[#0a1330]">{item.topic?.title_ar ?? "موضوع"}</span>
                        {item.is_exception && <span className="rounded-full bg-amber-50 px-2 py-0.5 text-[9px] font-black text-amber-700">استثنائي</span>}
                        {isAgendaEditable(selected.status) && <div className="flex gap-1"><button onClick={() => void moveAgendaItem(index,-1)} disabled={index === 0} className="rounded px-1.5 text-[10px] font-bold text-[#0066cc] disabled:opacity-30" title="رفع البند">↑</button><button onClick={() => void moveAgendaItem(index,1)} disabled={index === (selected.agenda_items?.length ?? 1)-1} className="rounded px-1.5 text-[10px] font-bold text-[#0066cc] disabled:opacity-30" title="خفض البند">↓</button></div>}
                        {isAgendaEditable(selected.status) && <button onClick={() => void removeAgendaItem(item.id)} className="text-[10px] font-bold text-red-600">إزالة</button>}
                      </div>
                    ))}
                  </div>
                ) : <p className="text-xs text-[#8291a4]">لا توجد بنود مضافة بعد.</p>}
              </div>
              ...
              {(["waiting_for_minutes", "waiting_for_approval", "closed"].includes(selected.status)) && (
                <div className="space-y-4 p-5">
                  <div className="flex items-center justify-between gap-3">
                    <div><h3 className="text-xs font-black text-[#0a1330]">محضر الاجتماع</h3><p className="mt-1 text-[10px] text-[#718196]">مسودة بشرية قابلة للمراجعة ثم المصادقة الرسمية.</p></div>
                    {minutes && <span className="rounded-full bg-[#edf6ff] px-2.5 py-1 text-[9px] font-black text-[#0066cc]">{minutes.status === "approved" ? "معتمد" : minutes.status === "ready_for_approval" ? "بانتظار المصادقة" : "مسودة"}</span>}
                  </div>
                  {minutesLoading ? <div className="grid min-h-28 place-items-center"><LoaderCircle className="animate-spin text-[#0066cc]" size={22} /></div> : <>
                    <textarea value={minutesText} onChange={(event) => setMinutesText(event.target.value)} disabled={selected.status !== "waiting_for_minutes"} placeholder="دوّن ملخص الحضور والنصاب ومناقشة البنود والتصويت والقرارات..." className="min-h-52 w-full rounded-2xl border border-[#dbe5ef] bg-white p-4 text-xs leading-7 outline-none focus:border-[#0066cc] disabled:bg-[#f7f9fb]" />
                    {selected.status === "waiting_for_minutes" && <div className="flex flex-wrap gap-2"><button onClick={() => void saveMinutesDraft()} disabled={minutesLoading || minutesText.trim().length < 20} className="rounded-xl border border-[#bfd5e8] px-4 py-2 text-[11px] font-bold text-[#0066cc] disabled:opacity-40">حفظ المسودة</button><button onClick={() => void submitMinutes()} disabled={minutesLoading || !minutes || minutesText.trim().length < 20} className="rounded-xl bg-[#0a1330] px-4 py-2 text-[11px] font-bold text-white disabled:opacity-40">إرسال للمصادقة</button></div>}
                    {!!minutes?.approvals?.length && <div className="space-y-2"><h4 className="text-[11px] font-black text-[#0a1330]">طلبات المصادقة</h4>{minutes.approvals.map((approval) => <div key={approval.id} className="flex flex-wrap items-center gap-2 rounded-xl border border-[#e5edf4] bg-[#fbfdff] p-3"><span className="flex-1 text-[11px] font-bold text-[#24364e]">{approval.name_ar ?? "صاحب صلاحية المصادقة"}</span><span className={`rounded-full px-2 py-1 text-[9px] font-black ${approval.approval_status === "approved" ? "bg-emerald-50 text-emerald-700" : approval.approval_status === "rejected" ? "bg-red-50 text-red-700" : "bg-amber-50 text-amber-700"}`}>{approval.approval_status === "approved" ? "تمت المصادقة" : approval.approval_status === "rejected" ? "أعيد للمراجعة" : "بانتظار الرد"}</span>{approval.approval_status === "pending" && <><button onClick={() => void respondApproval(approval,"approve")} className="rounded-lg bg-emerald-600 px-3 py-1.5 text-[10px] font-bold text-white">مصادقة</button><button onClick={() => void respondApproval(approval,"return")} className="rounded-lg border border-red-200 px-3 py-1.5 text-[10px] font-bold text-red-600">إعادة</button></>}</div>)}</div>}
                  </>}
                </div>
              )}
              ...
              <div className="flex flex-wrap gap-2 p-5">
                {selected.status === "draft" && <button onClick={() => transitionMeeting("scheduled")} className="flex items-center gap-1.5 rounded-xl bg-[#0066cc] px-3 py-2 text-[11px] font-bold text-white"><Play size={14} /> جدولة</button>}
                {selected.status === "scheduled" && <><button onClick={() => void sendInvitations()} disabled={!readiness?.ready} className="flex items-center gap-1.5 rounded-xl border border-[#bfd5e8] px-3 py-2 text-[11px] font-bold text-[#0066cc] disabled:opacity-40"><Users size={14} /> تجهيز الدعوات</button><button onClick={() => transitionMeeting("ready_to_start")} disabled={!readiness?.ready} className="flex items-center gap-1.5 rounded-xl bg-purple-600 px-3 py-2 text-[11px] font-bold text-white disabled:bg-[#a9b6c5]"><Users size={14} /> تجهيز الجلسة</button></>}
                {selected.status === "ready_to_start" && <button onClick={() => void openLiveSession()} className="flex items-center gap-1.5 rounded-xl bg-emerald-600 px-3 py-2 text-[11px] font-bold text-white"><Play size={14} /> فتح الجلسة الحية</button>}
                ...
                {["draft","scheduled","ready_to_start"].includes(selected.status) && <button onClick={() => void transitionMeeting("cancelled")} className="rounded-xl border border-red-200 px-3 py-2 text-[11px] font-bold text-red-600">إلغاء الاجتماع</button>}
                {selected.status === "closed" && <button onClick={() => void transitionMeeting("archived")} className="rounded-xl border border-[#cbd7e3] px-3 py-2 text-[11px] font-bold text-[#52647a]">أرشفة الاجتماع</button>}
              </div>
            </div>
          )}
        </div>
          ...
          <div className="w-full max-w-2xl overflow-hidden rounded-3xl bg-white shadow-2xl">
            ...
            <div className="p-6">
              ...
              <div className="mt-4 max-h-[45vh] space-y-2 overflow-y-auto">
                {agendaLoading ? <div className="grid min-h-36 place-items-center"><LoaderCircle className="animate-spin text-[#0066cc]" size={24} /></div> : agendaCandidates.length ? agendaCandidates.map((topic) => <div key={topic.id} className="flex flex-wrap items-center gap-3 rounded-2xl border border-[#e5edf4] bg-[#fbfdff] p-4"><div className="min-w-0 flex-1"><h3 className="truncate text-xs font-black text-[#0a1330]">{topic.title_ar}</h3><p className="mt-1 text-[10px] text-[#718196]">{topic.topic_no ?? "موضوع مؤهل"}{topic.priority ? ` · ${topic.priority}` : ""}</p>{topic.current_step && <p className="mt-1 text-[10px] font-bold text-[#0066cc]">الخطوة الحالية: {topic.current_step}</p>}</div><button type="button" onClick={() => void addAgendaItem(topic.id)} disabled={detailLoading} className="rounded-xl bg-[#0066cc] px-4 py-2 text-[11px] font-bold text-white disabled:opacity-50">إضافة إلى الجدول</button></div>) : <div className="rounded-2xl border border-dashed border-[#caddec] bg-[#fbfdff] p-8 text-center"><Search className="mx-auto text-[#84a4c2]" size={28} /><h3 className="mt-3 text-sm font-black text-[#193451]">لا توجد موضوعات مؤهلة حاليًا</h3><div className="mx-auto mt-3 max-w-lg rounded-xl bg-[#f5f9fd] p-3 text-right text-[10px] leading-6 text-[#60748a]"><p>يظهر الموضوع هنا عندما:</p><p>1. يكون معتمدًا.</p><p>2. تكون خطوته الحالية «عرض/مناقشة/توصية/اعتماد» وموجهة إلى جهة هذا الاجتماع.</p><p>3. يكون الاجتماع مسودة أو مجدولًا ولم يسبق إدراج الموضوع فيه.</p></div><Link href="/admin/topics" className="mt-3 inline-flex rounded-xl bg-[#0066cc] px-4 py-2 text-[11px] font-bold text-white">فتح الموضوعات والمراجعة</Link></div>}
              </div>
            </div>
