...
4. عند فتح بطاقة خيار، استدعِ `get_topic_regulation_preview` بالاختيار نفسه لعرض النص الرسمي للمادة، ملخص القواعد، النطاق، المسار، المتطلبات وأثره على الاعتماد والتصويت.
5. إذا كانت النتيجة الوحيدة، اخترها تلقائيًا مع إبقاء إمكانية تغييرها. أما النتائج المتعددة فتحتاج تأكيد المستخدم الصريح بزر «اختيار هذه اللائحة».
6. قبل التأكيد النهائي، استدعِ `get_topic_regulation_route_preview` واعرض الخطوات المسؤولة وشروط انتقالها بلغة تنفيذية واضحة.
7. إذا لم توجد خيارات، أوقف الإرسال واعرض حالة الحوكمة المناسبة. لا تنشئ موضوعًا غير منظم.
...
لا تعرض واجهة إنشاء الموضوع الاستثناء افتراضيًا. عند عدم وجود لائحة منطبقة، أو عند احتياج
اللائحة لمسار مخصص، أو عندما يسمح المسار غير المكتمل بالاستثناء، اقرأ
`get_topic_exception_workflow_options(p_governance_unit_id)` أولًا. يعيد العقد `can_request`
وقائمة آمنة للمستخدم من المسارات المؤقتة فقط؛ لا تستخدم عقد إدارة قوالب المسارات في هذه
الواجهة. إذا كانت `can_request=false` أو كانت القائمة فارغة، يبقى إنشاء الموضوع غير المحكوم
محظورًا ويُوجَّه المستخدم إلى مسؤول الحوكمة.
