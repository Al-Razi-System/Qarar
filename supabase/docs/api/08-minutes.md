...
## Snapshot-restored minutes commands

The meeting workspace calls `generate_meeting_minutes_draft` with `p_meeting_id` after the meeting
enters `waiting_for_minutes`. The command returns the minute id, status, generated draft content, and
the latest update timestamp. It is limited to the chair or council secretary by the database routine.

An assigned approver calls `sign_meeting_minutes_approval` with the approval id, signature strokes,
